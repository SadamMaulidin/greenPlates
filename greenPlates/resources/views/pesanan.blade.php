  ...
  <header id="header" class="header menu fixed-top d-flex align-items-center">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          ...
          <li class="nav-item dropdown">
            <a href="/co">
                <img class="shopping-cart-icon nav-link dropdown-toggle" src="{{ asset('assets/img/shopping_cart.png') }}" alt="">
            </a>
    ...
    <main>
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <img src="{{ asset('assets/img/menu/menu-item-1.png') }}" width= 500 alt="Gambar Produk">
                </div>
                <div class="col-md-6">
                    <h1 class="nama-produk">{{ $produk['nama_produk'] }}</h1>
                    <p class="harga"> Rp. {{ number_format($produk['harga'], 2, ',', '.') }}</p>
                    <p class="deskripsi">Deskripsi</p>
                    <p>{{ $produk['desk_produk'] }}</p>
                    <p>Penilaian: {{ $produk['penilaian'] }}</p>
                    <p>Ulasan: {{ $produk['ulasan'] }}</p>

                    <form method="post" action="">
                        @csrf
                        <input type="text" name="jumlah_pesan" class="form-control" required="">
                        <button type="submit" class="button-keranjang">Masukkan keranjang</button>
                    </form>
                </div>
            </div>
        </div>
